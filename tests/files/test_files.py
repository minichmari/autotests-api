...
class TestFiles:
    ...
    def test_get_file_with_incorrect_file_id(self, files_client: FilesClient):
        #Отправлен запрос на получение файла с incorrect-file-id
        file_id = "incorrect-file-id"
        response = files_client.get_file_api(file_id)
        response_data = ValidationErrorResponseSchema.model_validate_json(response.text)

        #Проверяем, что сервер вернул 422 - Unprocessable Entity
        assert_status_code(response.status_code, HTTPStatus.UNPROCESSABLE_ENTITY)

        # Проверяем, что в ответе содержится ошибка в path запроса, поле "file_id"
